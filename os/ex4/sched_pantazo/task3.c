...
void printQ(){
        ...
        printf("----------Printing Q------------\n");
        while(temp != NULL){
                printf("name : %s , pid = %ld , prio = %d \n", temp->name, (long)temp->mypid, temp->prio);
		temp = temp -> next;
        }
...
void enqueue(ProcessTable node){
...
{	
	...
	int result = IsItHigh();
	if(result == 1){
		while(temp != NULL &&  ((temp->prio) == 1 )){
			printf("name : %s | id =  %d | pid = %d | prio = %d \n", temp->name, temp->myid, temp->mypid,temp->prio);
			temp = temp -> next;
		}	
	}
	else{
...
sched_kill_task_by_id(int id)
{
	...
	ProcessTable *temp;
	temp = head;
	while((temp->myid != id) && (temp != NULL)){
	 	temp = temp -> next;
	}
	if((temp->myid) != id)
		printf("Process not found, returning...\n");
	else
...
static void
sched_prio(int id,int priority){
...
process_request(struct request_struct *rq)
{
	switch (rq->request_no) {
		...
		case REQ_HIGH_TASK:
			sched_prio(rq->task_arg,1);

		case REQ_LOW_TASK:
			sched_prio(rq->task_arg,0);
		default:
			return -ENOSYS;
	}
}
