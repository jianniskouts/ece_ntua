...
/* How many high processes are there?*/
int counter;
/*The HIGH queue*/
queue * high_q; 
int id = 0;
...
sched_print_tasks(void){
	...
	while(temp != NULL){
		...
		else if(temp->p->prio == 1){
			priority = "HIGH";
		}
		else{
			printf("error on print priority\n");
			exit(0);
		}
		printf("Tasks:\nid: %d | pid: %d,| name: %s | priority = %s \n",temp->p->myid,temp->p->pid,temp->p->name,priority);
		temp = temp->pre;
...
/*Set priority*/
static void
sched_priority(int id, int priority){
	node * temp ;
	temp = q->head;
	while(temp != NULL){
		if (temp->p->myid == id){
			temp->p->prio = priority;
			if(temp->p->prio == 1){
				enqueue(temp->p,high_q);
				counter++;
			}
			else{
				counter--;
